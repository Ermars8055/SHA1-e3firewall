#!/usr/bin/env python
"""
CLI tool for computing and verifying Collatz-SHA1 composite signatures.

This script can be run directly or through Django management commands.
It supports computing signatures for files, verifying signatures,
and testing for collisions between files.
"""
...
def compute_file_signature(file_path: str) -> str:
    """
    Compute the signature for a given file.
    ...
    """
    # Read file in chunks and compute signature
    chunks = list(stream_file_in_chunks(file_path))
    combined_data = b''.join(chunks)
    return collatz_sha1_signature_of_data(combined_data)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Collatz-SHA1 Composite Signature Tool',
